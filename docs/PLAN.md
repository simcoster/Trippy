...
**`just` standardized on `scrape-*`.** `populate-reviews` → `scrape-reviews`, `ingest-rules` → `scrape-rules`. Entries below use the old names and were accurate when written. Four old mentions in this log were find-and-replaced before the append-only rule was stated — revert pending a decision.
