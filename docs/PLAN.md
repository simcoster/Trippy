...
**Open.** Existing tests written for two categories need updating and were left untouched pending permission: `test_rules_extraction.py` (`SubjectCategory.RULE` for `check_in` / `check_out` / `night` / `age`, now numeric) and `test_subject_adjudication_llm.py::test_classify_assigns_the_right_category` (`min_weekend_nights`, `check_out_time` → 3).
