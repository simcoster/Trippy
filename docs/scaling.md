...
- **CI:** `pytest -m "not llm"` on every PR. Alembic `upgrade` against a throwaway Postgres. Dummy `NEBIUS_API_KEY` / `TELEGRAM_TOKEN` only so import-time constructors do not raise; they are not real secrets and Telegram is not called. Scraper code requests `OpenAI` only via `make_nebius_openai_client`, which raises `LiveLlmDisabled` unless production or the test is marked `llm`.
